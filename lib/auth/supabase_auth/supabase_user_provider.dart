...
class ProExDoacoesSupabaseUser extends BaseAuthUser {
  ProExDoacoesSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> proExDoacoesSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ProExDoacoesSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
